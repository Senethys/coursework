 ...
 */
 void is_not_empty_test() {
 	...
 	*v=1;
 	...
 	if (stack_is_empty(s)) {
 		fprintf(stderr, "FAIL: New stack is empty after push!\n");
 		stack_kill(s);
 		exit(EXIT_FAILURE);
 	}
 	free_all_types(s);
...
/**
	* This function populates stack and checks if top works appropriately.
	* Returns: void
	*
	*
	*/
  ...
  *numb = 41;
  ...
  //Check top and remove it.
	...
	//Check top and remove it.
 	...
 	//stack_print(s, print_ints);
 	if(*top_bool != 0) {
 		fprintf(stderr, "FAIL: The stack got modified after pop!\n");
	...
	//stack_print(s, print_ints);
 	if((*top_int != compare_char)) {
 		...
 		fprintf(stderr, "Value: %i. Should be: 116 (int for 't')", *top_int);
 		stack_kill(s);
 		exit(EXIT_FAILURE);
 	}
 	free(top_int);
